...
async function seedExpenseCategories() {
    const defaults = ['Grocery', 'Rent', 'Electricity'];
...
if (expenseForm) {
    expenseForm.onsubmit = async (e) => {
        ...
        console.log("Submitting expense:", { id, name, category, amount, date });

        try {
            if (id) {
                const docRef = doc(db, "expenses", id);
                await updateDoc(docRef, { name, category, amount, date, lastUpdated: new Date() });
                showToast("✅ Expense Updated!");
            } else {
                await addDoc(collection(db, "expenses"), { name, category, amount, date, timestamp: new Date() });
